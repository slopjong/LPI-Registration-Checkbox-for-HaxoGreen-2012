mediaWiki.loader.implement("ext.Registration", function ($, mw) {
    function checkForm() {
        var price = $('#price').html();
        if ($('#firstname').val() == '' || $('#lastname').val() == '' || $('#email').val() == '') {
            alert('You need to fill out at least your first- and lastname, as well as your email address.');
            return false;
        }
        ...
        if (confirm("Please confirm your registration:\nFee: " + price)) {
            $('#button_submit').closest('form').submit();
        }
    }
    ...
    function shirts() {
        ...
        if (amount > 9) {
            alert('Sorry, dude, we don\'t even have that many shirts!');
            return false;
        }
        var pattern = /^[0-9]{1}$/;
        ...
        if (window.mediaWiki) {
            var available = window.settings.
            shirtsizes.split(',');
            var i = $("#sizes div").length;
            if (i < amount) {
                for (; i < amount; i++) {
                    var div = $('<div>');
                    var select = document.createElement('select');
                    select.name = 'shirt_sz[]';
                    select.id = 'shirtsel_' + i;
                    $.each(available, function (k, v) {
                        $(select).append('<option value="' + v + '">' + v + '</option>');
                    });
                    div.append(select);
                    if (window.settings.gendered == "true") {
                        div.append(genderSelector(i));
                    }
                    div.append('<br/>');
                    $("#sizes").append(div);
                }
            } else if (amount >= 0) {
                while ($("#sizes div").length > amount) {
                    $("#sizes div").last().remove();
                }
            }
        }
        calcPrice();
    }
    function genderSelector(shirtId) {
        var gender = document.createElement('select');
        gender.name = 'shirt_gdr[]';
        gender.id = 'gendersel_' + shirtId;
        $.each(['M', 'F'], function (k, v) {
            $(gender).append('<option value="' + v + '">' + v + '</option>');
        });
        return gender;
    }
    function calcPrice() {
        var shirtPrice = parseFloat(window.settings.shirtfee);
        var shirtNum = parseInt($('#amount').val());
        var shirtTotal = shirtNum * shirtPrice;
        var workshopPrice = parseFloat($('#workshopfee').val());
        var phase = window.settings.phase;
        var price = 0;
        if ($('#supporter').attr('checked')) {
            price = parseFloat(window.settings.supporter);
        } else if (phase == 'EarlyBird phase') {
            price = parseFloat(window.settings.ebfee);
        } else if (phase == 'Late phase') {
            price = parseFloat(window.settings.ltfee);
        } else {
            alert('phase "' + phase + '" not supported');
        }
        if (!isNaN(workshopPrice)) price += workshopPrice;
        if (!isNaN(shirtTotal)) price += shirtTotal;
        $("#price").html(price + " EUR");
    }
    function changeWSFee(newprice) {
        var wsprice = parseFloat($('#workshopfee').val());
        wsprice += newprice;
        $('#workshopfee').val(wsprice);
        calcPrice();
    }
	...
	/*
        $('#workshop-0').click(function () {
            var price = this.getAttribute('itemprop');
            if (this.checked) {
                changeWSFee(price);
                alert('You won\'t have to pay for the LPIC during registration, but on-site.\n' + 'I.e. this will NOT be free despite the 0.0 price tag\n' + 'However, we will fund the first 12 people to sign up with 40 EUR.\n' + 'See the link for more details.');
            } else {
                changeWSFee(-1 * price);
            }
        });
	*/
    ...
    "all": ".reg_input{border:1px solid #005580;width:25em;font-family:Helvetica,Arial,sans-serif} .reg_number{width:3em;border:1px solid #005580;font-family:Helvetica,Arial,sans-serif}.reg_cell{border-bottom:1px solid #d5d5d3;padding:5px}.reg_label{text-transform:uppercase;padding:5px;font-size:8pt;font-weight:bold;margin-right:90px}.reg_table{border:1px solid #d5d5d3}.reg_button{background-color:white;border:1px solid #005580}.reg_button_cell{text-align:right} .reg_error{font-size:larger;padding:10px;margin:20px;border:4px solid red}"
}, {});
    ...
    (function ($) {
        ...
        $.fn.autoEllipsis = function (options) {
            ...
            $(this).each(function () {
                ...
                if (
                options.matchText && !(options.matchText in matchTextCache[text])) {
        ...
        jQuery.fn.checkboxShiftClick = function (text) {
            ...
            $box.click(function (e) {
                if (prevCheckbox !== null && e.shiftKey) {
                    $box.slice(Math.min($box.index(prevCheckbox), $box.index(e.target)), Math.max($box.index(prevCheckbox), $box.index(e.target)) + 1).attr({
...
mediaWiki.loader.implement("jquery.client", function ($, mw) {
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            $('html').addClass('client-' + profile.name).addClass('client-' + profile.name + '-' + profile.versionBase).addClass('client-' + profile.layout).addClass('client-' + profile.platform);
        });
    })(jQuery);;
}, {}, {});
    ...
    (function ($) {
        ...
        $.collapsibleTabs = {
            ...
            moveToCollapsed: function (ele) {
                ...
                $moving.detach().prependTo(data.collapsedContainer).data('collapsibleTabsSettings', data);
...
mediaWiki.loader.implement("jquery.delayedBind", function ($, mw) {
    (function ($) {
...
mediaWiki.loader.implement("jquery.expandableField", function ($, mw) {
    (function ($) {
        $.expandableField = {
            expandField: function (e, context) {
                context.config.beforeExpand.call(context.data.$field, context);
                context.data.$field.animate({
                    'width': context.data.expandedWidth
                }, 'fast', function () {
            ...
            condenseField: function (e, context) {
                ...
                context.data.$field.animate({
                    'width': context.data.condensedWidth
                }, 'fast', function () {
            ...
            configure: function (context, property, value) {
                switch (property) {
                    default: context.config[property] = value;
                    break;
                }
            }
        };
        $.fn.expandableField = function () {
            ...
            $(this).each(function () {
                ...
                if (typeof context.data == 'undefined') {
                    ...
                    $(this).addClass('expandableField').focus(function (e) {
    ...
    (function ($) {
        $.highlightText = {
            ...
            innerHighlight: function (node, pat) {
                ...
                } else if (node.nodeType == 1 && node.
                childNodes && !/(script|style)/i.test(node.tagName) && !(node.tagName.toLowerCase() == 'span' && node.className.match(/\bhighlight/))) {
            ...
            $(this).each(function () {
                ...
                if (typeof context.data == 'undefined') {
                    ...
                    if (context.config.positionFromLeft) {
                        ...
                        newCSS['left'] = 'auto';
                        newCSS['right'] = $('body').
                        width() - (context.config.$region.offset().left + context.config.$region.outerWidth());
                    }
...
mediaWiki.loader.implement("mediawiki.legacy.mwsuggest", function ($, mw) {
    if (!mw.config.exists('wgMWSuggestTemplate')) {
        mw.config.set('wgMWSuggestTemplate', mw.config.get('wgServer') + mw.config.get('wgScriptPath') + "/api.php?action=opensearch\x26search={searchTerms}\x26namespace={namespaces}\x26suggest");
    }
    window.os_map = {};
    window.os_cache = {};
    ...
    window.os_encodeQuery = function (
    value) {
    ...
    window.os_getNamespaces = function (r) {
        ...
        for (i = 0; i < elements.length; i++) {
            ...
            if (typeof name != 'undefined' && name.length > 2 && name[0] == 'n' && name[1] == 's' && ((elements[i].type == 'checkbox' && elements[i].checked) || (elements[i].type == 'hidden' && elements[i].value == '1'))) {
                if (namespaces != '') {
                    namespaces += '|';
                }
    ...
    window.os_delayedFetch = function () {
        ...
        os_timer = null;
        var path = mw.config.get('wgMWSuggestTemplate').replace("{namespaces}", os_getNamespaces(r)).replace("{dbname}", wgDBname).replace("{searchTerms}", os_encodeQuery(query));
    ...
    window.os_createToggle = function (r, className) {
        ...
        link.onclick = function () {
            os_toggle(r.searchbox, r.searchform);
        };
...
mediaWiki.loader.implement("mediawiki.legacy.wikibits", function ($, mw) {
    ...
    window.is_gecko = /gecko/.test(clientPC) && !/khtml|spoofer|netscape\/7\.0/.test(clientPC);
    window.is_safari = window.is_safari_win = window.webkit_version = window.is_chrome = window.is_chrome_mac = false;
    window.webkit_match = clientPC.match(/applewebkit\/(\d+)/);
    if (webkit_match) {
        window.is_safari = clientPC.indexOf('applewebkit') != -1 && clientPC.indexOf('spoofer') == -1;
        window.is_safari_win = is_safari && clientPC.indexOf('windows') != -1;
        window.webkit_version = parseInt(webkit_match[1]);
        window.is_chrome = clientPC.indexOf('chrome') !== -1 && clientPC.indexOf('spoofer') === -1;
    ...
    window.is_opera = window.is_opera_preseven = window.is_opera_95 = window.opera6_bugs = window.opera7_bugs = window.opera95_bugs = false;
    ...
    if (/msie ([0-9]{1,}[\.0-9]{0,})/.exec(clientPC) != null && parseFloat(RegExp.$1) <= 6.0) {
        ie6_bugs = true;
    }
    ...
    window.importScript = function (page) {
        var uri = wgScript + '?title=' + encodeURIComponent(page.replace(/ /g, '_')).replace(/%2F/ig, '/').replace(/%3A/ig, ':') + '&action=raw&ctype=text/javascript';
    ...
    window.showTocToggle = function () {
        if (document.createTextNode) {
            ...
            toggleLink.href = '#';
            addClickHandler(toggleLink, function (evt) {
                toggleToc();
                return killEvt(evt);
            });
            toggleLink.appendChild(
            document.createTextNode(mediaWiki.msg('hidetoc')));
    ...
    window.updateTooltipAccessKeys = function (nodeList) {
        if (!nodeList) {
            var linkContainers = ['column-one', 'mw-head', 'mw-panel', 'p-logo'];
    ...
    window.ts_makeSortable = function (table) {
        ...
        for (var i = 0; i < firstRow.cells.length; i++) {
            ...
            if ((' ' + cell.className + ' ').indexOf(' unsortable ') == -1) {
                $(cell).append('<a href="#" class="sortheader" ' + 'onclick="ts_resortTable(this);return false;">' + '<span class="sortarrow">' + '<img src="' + ts_image_path + ts_image_none + '" alt="&darr;"/></span></a>');
            }
        }
    ...
    window.ts_resortTable = function (lnk) {
        ...
        if (rowStart == 0 && table.tBodies) {
            ...
            if (bodyRows < table.rows.length) rowStart = 1;
        }
        var itm = '';
    ...
    window.ts_initTransformTable = function () {
        if (typeof wgSeparatorTransformTable == 'undefined' || (wgSeparatorTransformTable[0] == '' && wgDigitTransformTable[2] == '')) {
            ...
            for (var digit in ts_number_transform_table) {
                digits.push(digit.replace(/[\\\\$\*\+\?\.\(\)\|\{\}\[\]\-]/, function (s) {
                    return '\\' + s;
                }));
    ...
    window.jsMsg = function (message, className) {
        ...
        if (!messageDiv) {
            ...
            if (document.getElementById('column-content') && document.getElementById('content')) {
                ...
                insertBefore(messageDiv, document.getElementById('content').firstChild);
            } else if (document.getElementById('content') && document.getElementById('article')) {
                document.getElementById('article').insertBefore(messageDiv, document.getElementById('article').firstChild);
            } else {
                return false;
            }
        }
